...
export function BracketPairing({
  ...
  if (round === 1) {
    return (
      <Matchup
        bracketSlots={[
          bracket[2 * parentIndex + 1],
          bracket[2 * parentIndex + 2],
        ]}
        winnerSlot={bracket[parentIndex]}
        parentIndex={parentIndex}
        currentMatchup={currentMatchup}
        onPick={onPick}
        size={size[round]}
        showOutgoingLine={true}
      />
    );
  }
  ...
  if (round > 1) {
    ...
    return (
      <div className="flex flex-row items-center">
        ...
        <div
          className="self-stretch flex flex-col w-10"
        ...
        <Matchup
          ...
          currentMatchup={currentMatchup}
          onPick={onPick}
          size={size[round as keyof typeof size]}
          showIncomingLine={true}
          showOutgoingLine={round < 4}
        />
      </div>
    );
  }
