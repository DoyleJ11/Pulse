...
export function BracketView() {
  ...
  // Cumulative X positions per round.
  const xs: number[] = [0];
  for (let i = 0; i < 3; i++) {
    xs.push(xs[i] + COL_W + COL_GAP);
  }
  const championX = xs[3] + COL_W + COL_GAP;
  const totalW = championX + CHAMPION_W;
    ...
    <div className="flex flex-col">
      ...
      <div className="m-6 p-4">
        <div className="overflow-x-auto">
          <div className="flex gap-100 ml-4">
            {HEADERS.map((header) => {
              const isIconSubtitle = typeof header.subtitle !== "string";
              return (
                <div
                  key={header.title}
                  className="my-10 w-91"
                  style={{ width: CARD_W }}
                >
                  <div
                    className="flex justify-between items-baseline h-14 rounded-xl px-5.5 py-3.5 shadow-[3px_3px_0px_0px_#0A0A0A] border-solid border-2 tracking-widest box-border"
